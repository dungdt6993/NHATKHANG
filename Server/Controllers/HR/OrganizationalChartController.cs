...
namespace D69soft.Server.Controllers.HR
{
    ...
    public class OrganizationalChartController : ControllerBase
    {
        ...
        public async Task<ActionResult<int>> UpdateDepartment(DepartmentVM _departmentVM)
        {
            ...
            {
                sql += "Insert into HR.Department (DepartmentID,DepartmentName,DepartmentGroupID,ShiftID,StockCode,DivisionID,isActive) Values (@DepartmentID,@DepartmentName,@DepartmentGroupID,@ShiftID,@StockCode,@DivisionID,@isActive) ";
            }
            if (_departmentVM.IsTypeUpdate == 1)
            {
                sql += "Update HR.Department set DepartmentName = @DepartmentName,DepartmentGroupID = @DepartmentGroupID, ShiftID = @ShiftID, StockCode = @StockCode, isActive = @isActive where DepartmentID = @DepartmentID ";
            }
